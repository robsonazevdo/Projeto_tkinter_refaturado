...
class Historico_Atendimento(tk.Toplevel):
    def __init__(self, parent):
        ...
        # Quadro para Treeview de Clientes (mover para o topo)
        ...
        self.tv2 = ttk.Treeview(self.quadroGrid, columns=("id", "Nome"), show="headings", height=10)  # Reduzido o valor de height para ajustar
        ...
        # Quadro para Treeview de Atendimentos (mover para baixo dos clientes)
        ...
        btn_carregar_clientes = tk.Button(
            ...
            bd=5,
            command=lambda: self.pesquisaCliente(self.pNome.get())
    ...
    def alterar(self):
        try:
            ...
            if edition_position[0] != '' and edition_position[1] != '':
                self.editar_atendimento(edition_position, self.cliente, self.id_comanda)
                self.destroy()
            else:
                showerror("ERRO!", "Escolha um Cliente", parent=self)   
        except:
            showerror("ERRO!", "Escolha um Cliente", parent=self)

    def editar_atendimento(self, edition_position, cliente, id_comanda):
        Editar_Historico(self.master, edition_position, cliente, id_comanda)
